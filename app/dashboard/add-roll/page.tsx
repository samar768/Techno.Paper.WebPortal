...
export default function AddRollPage() {
	...
	const validateForm = (): string[] => {
		...
		if (!formData.weight.trim() || Number.parseFloat(formData.weight) <= 0)
	...
	const handleSubmit = useCallback(() => {
		...
		toast.success(`${formData.sku} has been added to inventory.`);
	...
	const generateQRCode = useCallback(() => {
		const randomId = `QR${Math.random()
			.toString(36)
			.substr(2, 6)
			.toUpperCase()}`;
		handleInputChange('qrBarcodeId', randomId);

		toast.success(`Generated QR Code: ${randomId}`);
